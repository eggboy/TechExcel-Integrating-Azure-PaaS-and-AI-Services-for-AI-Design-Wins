...
def handle_query_vectorization(query):
    ...
    response = requests.get(
        f"{api_endpoint}/Vectorize", params={"text": query}, timeout=10, verify=False
    )
...
def main():
    ...
    with col2:
        max_results = st.text_input(
            "Max results (<=0 will return all results):", key="max_results", value=0
        )

    minimum_similarity_score = st.slider(
        "Minimum Similarity Score:", min_value=0.0, max_value=1.0, value=0.8, step=0.01
    )
    ...
    if st.button("Submit"):
        with st.spinner("Performing vector search..."):
            if query:
                ...
                vector_search_results = handle_vector_search(
                    query_vector, max_results, minimum_similarity_score
                )
                # Display the results.
                st.write("## Results")
